...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_getDataMMOverdue]
...
SET @sql=@sql+'SELECT rowid,
       ...
       cnt_acc,
       min_overdue_days,
       max_overdue_days,'''' AS deskripsi_jenis_ljk FROM dbo.BDA_F01_MaxMinOverdue WHERE rowid=0'
END 
ELSE	
BEGIN
SET @sql=@sql+'
...
END 
EXEC sys.sp_executesql @sql, N'@jenis_ljk NVARCHAR(MAX), @ljk NVARCHAR(MAX), @periode_awal VARCHAR(20), @periode_akhir VARCHAR(20),@jenisDebitur VARCHAR(200), @statusCollectability VARCHAR(100) ', @jenis_ljk, @ljk, @periode_awal, @periode_akhir, @jenisDebitur, @statusCollectability
PRINT @sql
GO
