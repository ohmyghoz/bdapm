...
CREATE PROCEDURE [dbo].[usp_getSummaryJenisLJK]
	 @SelectedDQ NVARCHAR(MAX),
...
BEGIN
    ...
    SELECT 
		...
		TableName,
        a.JenisLJK, 
        a.KodeLJK + '' - '' + c.nama_ljk AS NamaLJK,
		a.KodeLJK,
        a.Period,
        a.Rows 
    ...
    IF @SelectedDQ IS NOT NULL
    BEGIN
        SET @DynamicSQL = @DynamicSQL + ' AND a.TableName IN (' + @SelectedDQ + ')'
    END
