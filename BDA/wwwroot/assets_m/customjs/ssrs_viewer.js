...
function reloadParameters() {
	var params = $('.Parameters :input').serializeArray();
	var urlParams = $.param(params);
	showLoadingProgress("Updating Parameters...");
	$.get("/SSRS/Demo1/ReloadParameters/?reportPath=@Model.ReportPath.UrlEncode()&" + urlParams).done(function (data) {
		if (data != null) {
...
function printReport() {
	var params = $('.Parameters :input').serializeArray();
	var urlParams = $.param(params);
	window.open("/SSRS/Demo1/PrintReport/?reportPath=@Model.ReportPath.UrlEncode()&" + urlParams, "_blank");
}
function findText() {
	...
	if (searchText != undefined && searchText != null && searchText != "") {
		...
		$.get("/SSRS/Demo1/FindStringInReport/?reportPath=@Model.ReportPath.UrlEncode()&page=" + page + "&searchText=" + searchText + "&" + urlParams).done(function (data) {
			if (data > 0) {
...
function viewReportPage(page, afterReportLoadedCallback) {
	...
	$.get("/SSRS/Demo1/ViewReportPage/?reportPath=@Model.ReportPath.UrlEncode()&page=" + page + "&" + urlParams)
		.done(function (data) {
...
function exportReport(element) {
	var params = $('.Parameters :input').serializeArray();
	var urlParams = $.param(params);
	var format = $(element).attr('class').replace("Export", "").replace("btn btn-default ", "");
	window.location.href = "/SSRS/Demo1/ExportReport/?reportPath=@Model.ReportPath.UrlEncode()&format=" + format + "&" + urlParams;
}
