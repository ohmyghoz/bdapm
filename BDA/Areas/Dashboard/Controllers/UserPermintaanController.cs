...
{
    [Area("Dashboard")]
    public class UserPermintaanController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public UserPermintaanController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index(string lvl, string from, string until, string type, string member, string datamart, string ideb)
        {
            ...
            db.CheckPermission("User Permintaan View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);

            db.InsertAuditTrail("UserPermintaan_Akses_Page", "Akses Page Dashboard User Permintaan", pageTitle);

            //var roleId = HttpContext.User.FindFirst(ClaimTypes.Role).Value;
            //ViewBag.Akses = roleId;
            ViewBag.Export = db.CheckPermission("User Permintaan Export", DataEntities.PermissionMessageType.NoMessage);
        ...
        [HttpPost]
        public IActionResult Detail(string memberType, string member, string userId)
        {
            ...
            return Json(new { result = "Redirect", url = Url.Action("Detail", "UserPermintaan") });
        }
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("User Permintaan Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_UserPermintaan", "Export Data", pageTitle);
        ...
        public IActionResult LogExportDetail()
        {
            try
            {
                ...
                db.CheckPermission("User Permintaan Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportDetail_UserPermintaan", "Export Data", pageTitle);
                return Json(new { result = "Success" });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult LogExportInquiry()
        {
            try
            {
                ...
                db.CheckPermission("User Permintaan Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportInquiry_UserPermintaan", "Export Data", pageTitle);
                return Json(new { result = "Success" });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Detail()
        {
            ...
            var listGlo = (from q in db.Glossary
                           where q.stsrc == "A" && q.GloPIdeb == pideb && q.GloTipe == "UP"
                           select q).ToList();
            if (listGlo.Count() != 0)
            {
        ...
        public IActionResult ExportPDF(IFormFile file)
        {
            try
            {
                ...
                var fileName = "UserPermintaan_" + timeStamp + ".pdf";
        ...
        #region "GetGridData"
        public object GetGridData(DataSourceLoadOptions loadOptions, string levelAlert, string memberTypes, string members, string periodeAwal, string periodeAkhir, string dataMart, string permintaanIDeb)
        {
            TempData.Clear();
            ...
            var query = db.getGroupedAlertSummary("UP", null, stringLevelAlert, stringMemberTypes, stringMembers, null, stringPeriodeAwal, stringPeriodeAkhir, dataMart, permintaanIDeb, null, null, null, null).ToList();
        ...
        public object GetChartData(DataSourceLoadOptions loadOptions)
        {
            ...
            db.Database.CommandTimeout = 420;
            var query = db.getAlertSummary("UP", null, stringLevelAlert, memberType, member, null, stringPeriodeAwal, stringPeriodeAkhir, dataMart, permintaanIDeb, userId, null, null, null).ToList();
