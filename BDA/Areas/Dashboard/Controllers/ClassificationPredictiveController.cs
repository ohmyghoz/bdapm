...
{
    [Area("Dashboard")]
    public class ClassificationPredictiveController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public ClassificationPredictiveController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index()
        {
            ...
            db.CheckPermission("Classification Predictive Actual Collectability View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
            ViewBag.Export = db.CheckPermission("Classification Predictive Actual Collectability Export", DataEntities.PermissionMessageType.NoMessage);

            db.InsertAuditTrail("ClassificationPredictive_Akses_Page", "Akses Page Dashboard Classification Predictive Actual Collectability", pageTitle);
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("Classification Predictive Actual Collectability Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_ClassificationPredictive", "Export Data", pageTitle);
        ...
        public IActionResult LogExportDetail()
        {
            try
            {
                ...
                db.CheckPermission("Classification Predictive Actual Collectability Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportDetail_ClassificationPredictive", "Export Data", pageTitle);
                return Json(new { result = "Success" });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Detail(long? id)
        {
            ...
            var obj = db.BDA_F01_Predictive.Find(id);
            if (obj == null) return NotFound();

            ViewBag.Export = db.CheckPermission("Classification Predictive Actual Collectability Export", DataEntities.PermissionMessageType.NoMessage);

            db.InsertAuditTrail("ClassificationPredictive_Akses_Detail", "Akses Detail Dashboard Classification Predictive Actual Collectability", pageTitle);
        ...
        public IActionResult ExportPDF(IFormFile file)
        {
            try
            {
                ...
                var fileName = "ClassificationPredictive_" + timeStamp + ".pdf";
        ...
        public IActionResult Antrian(string reportId)
        {
            try
            {
                ...
                newq.rgq_tablename = "BDA_F01_Predictive";
                var isHive = Helper.WSQueryStore.IsPeriodInHive(db, "BDA_F01_Predictive");
                string stringMemberTypes = null;
                string stringMembers = null;
                string stringJD = null;
                string stringColle = null;
                string stringDColle = null;
                string stringPeriodeAwal = null;
                string stringPeriodeAkhir = null;
                if (reportId == "classification_predictive")
                {
                    newq.rgq_nama = "Classification Predictive Actual Collectability Export CSV";
                    ...
                    if (TempData.Peek("dc") != null)
                    {
                        stringDColle = TempData.Peek("dc").ToString();
                    }
                    ...
                    newq.rgq_query = Helper.WSQueryExport.GetClassificationPredictiveQuery(db, stringMemberTypes, stringMembers, stringJD, stringColle, stringDColle, stringPeriodeAwal, stringPeriodeAkhir, isHive);
        ...
        #region "GetGridData"
        public object GetGridData(DataSourceLoadOptions loadOptions, string memberTypes, string members, string periodeAwal, string periodeAkhir, string jenisDebitur, string statusCollectability, string dugaanCollectability)
        {
            ...
            string[] DugaanCollectability = JsonConvert.DeserializeObject<string[]>(dugaanCollectability);
            ...
            if (DugaanCollectability.Length > 0)
            {
                stringDColle = string.Join(", ", DugaanCollectability);
                TempData["dc"] = stringDColle;
            }

            db.Database.CommandTimeout = 420;

            var query = db.getDataClaPredictive(stringPeriodeAwal, stringPeriodeAkhir, stringMemberTypes, stringMembers, stringJD, stringColle,stringDColle).ToList();
        ...
        public object GetDetailData(DataSourceLoadOptions loadOptions, long? id)
        {
            try
            {
                ...
                var as1 = db.BDA_F01_Predictive.Find(id);
                ...
                string q = "SELECT account_no,cif,member_code,concat(substr(periode,0,4),'-', substr(periode,5,2) ,'-01') periode,collectibility_type_code,collectibility_predictive,desc_jenis_ljk_byreff,qty_all,qty_distinct_acc_cif,min_overdue_days ,max_overdue_days,sum_outstanding FROM f01_predictive_detail  " + System.Environment.NewLine;
                //string q = "Select * from inquiry_enh_det ied" + System.Environment.NewLine;

                q = q + "WHERE pperiode = '" + as1.periode.ToString("yyyyMM") + "' AND member_type_code='" + as1.member_type_code + "' AND member_code='" + as1.member_code + "' and collectibility_type_code='" + as1.collectibility_type_code + "' and status='" + as1.status + "' and collectibility_predictive='" + as1.dugaan_collectability+"' " + System.Environment.NewLine;
                q = q + "LIMIT 100000 ";
                ...
                try
                {
                    ...
                    var query = (from row in dt.AsEnumerable()
                                 select new
                                 {
                                     ...
                                     collectibility_predictive = Convert.ToString(row["collectibility_predictive"]),
