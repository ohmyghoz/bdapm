...
{
    [Area("Dashboard")]
    public class RentangWaktuPermintaanController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public RentangWaktuPermintaanController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index(string lvl, string from, string until, string type, string member, string datamart, string ideb)
        {
            ...
            db.CheckPermission("Rentang Waktu Permintaan View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);

            db.InsertAuditTrail("RentangWaktuPermintaan_Akses_Page", "Akses Page Dashboard Rentang Waktu Permintaan", pageTitle);

            //var roleId = HttpContext.User.FindFirst(ClaimTypes.Role).Value;
            //ViewBag.Akses = roleId;
            ViewBag.Export = db.CheckPermission("Rentang Waktu Permintaan Export", DataEntities.PermissionMessageType.NoMessage);
        ...
        public IActionResult Detail(string memberType, string member, string kodeAlert)
        {
            ...
            return Json(new { result = "Redirect", url = Url.Action("Detail", "RentangWaktuPermintaan") });
        }
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("Rentang Waktu Permintaan Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_RentangWaktuPermintaan", "Export Data", pageTitle);
        ...
        public IActionResult LogExportDetail()
        {
            try
            {
                ...
                db.CheckPermission("Rentang Waktu Permintaan Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportDetail_RentangWaktuPermintaan", "Export Data", pageTitle);
                return Json(new { result = "Success" });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult LogExportInquiry()
        {
            try
            {
                ...
                db.CheckPermission("Rentang Waktu Permintaan Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportInquiry_RentangWaktuPermintaan", "Export Data", pageTitle);
                return Json(new { result = "Success" });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Detail()
        {
            ...
            var listGlo = (from q in db.Glossary
                           where q.stsrc == "A" && q.GloPIdeb == pideb && q.GloTipe == "RWP" && q.GloDimensi1 == di1
                           select q).ToList();
            if (listGlo.Count() != 0)
            {
        ...
        public IActionResult ExportPDF(IFormFile file)
        {
            try
            {
                ...
                var fileName = "RentangWaktuPermintaan_" + timeStamp + ".pdf";
        ...
        #region "GetGridData"
        public object GetGridData(DataSourceLoadOptions loadOptions, string levelAlert, string memberTypes, string members, string periodeAwal, string periodeAkhir, string dataMart, string permintaanIDeb, string waktu)
        {
            TempData.Clear();
            ...
            var query = db.getGroupedAlertSummary("RWP", waktu, stringLevelAlert, stringMemberTypes, stringMembers, null, stringPeriodeAwal, stringPeriodeAkhir, dataMart, permintaanIDeb, null, null, null, null).ToList();
        ...
        public object GetChartData(DataSourceLoadOptions loadOptions)
        {
            ...
            db.Database.CommandTimeout = 420;
            var query = db.getAlertSummary("RWP", kodeAlert, stringLevelAlert, memberType, member, null, stringPeriodeAwal, stringPeriodeAkhir, dataMart, permintaanIDeb, null, null, null, null).ToList();
