...
{
    [Area("Dashboard")]
    public class SizeBisnisPelaporController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public SizeBisnisPelaporController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index(string lvl, string from, string until, string type, string member, string datamart, string ideb)
        {
            ...
            db.CheckPermission("Size Bisnis Pelapor View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);

            db.InsertAuditTrail("SizeBisnisPelapor_Akses_Page", "Akses Page Dashboard Size Bisnis Pelapor", pageTitle);
        ...
        public IActionResult Detail(string memberType, string member, string kodeAlert)
        {
            ...
            return Json(new { result = "Redirect", url = Url.Action("Detail", "SizeBisnisPelapor") });
        }
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("Size Bisnis Pelapor Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_SizeBisnisPelapor", "Export Data", pageTitle);
        ...
        public IActionResult LogExportDetail()
        {
            try
            {
                ...
                db.CheckPermission("Size Bisnis Pelapor Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportDetail_SizeBisnisPelapor", "Export Data", pageTitle);
                return Json(new { result = "Success" });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult LogExportInquiry()
        {
            try
            {
                ...
                db.CheckPermission("Size Bisnis Pelapor Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportInquiry_SizeBisnisPelapor", "Export Data", pageTitle);
                return Json(new { result = "Success" });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Detail()
        {
            ...
            var listGlo = (from q in db.Glossary
                           where q.stsrc == "A" && q.GloPIdeb == pideb && q.GloTipe == "SB" && q.GloDimensi1 == di1
                           select q).ToList();
            if (listGlo.Count() != 0)
            {
        ...
        public IActionResult ExportPDF(IFormFile file)
        {
            try
            {
                ...
                var fileName = "SizeBisnisPelapor_" + timeStamp + ".pdf";
        ...
        #region "GetGridData"
        public object GetGridData(DataSourceLoadOptions loadOptions, string levelAlert, string memberTypes, string members, string periodeAwal, string periodeAkhir, string dataMart, string permintaanIDeb, string kategori)
        {
            TempData.Clear();
            ...
            var query = db.getGroupedAlertSummary("SB", kategori, stringLevelAlert, stringMemberTypes, stringMembers, null, stringPeriodeAwal, stringPeriodeAkhir, dataMart, permintaanIDeb, null, null, null, null).ToList();
        ...
        public object GetChartData(DataSourceLoadOptions loadOptions, long id)
        {
            ...
            db.Database.CommandTimeout = 420;
            var query = db.getAlertSummary("SB", kodeAlert, stringLevelAlert, memberType, member, null, stringPeriodeAwal, stringPeriodeAkhir, dataMart, permintaanIDeb, null, null, null, null).ToList();
