...
{
    [Area("Dashboard")]
    public class HMLMCDFAPeroranganController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public HMLMCDFAPeroranganController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index()
        {
            ...
            db.CheckPermission("HML MCDFA Perorangan View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
            //var roleId = HttpContext.User.FindFirst(ClaimTypes.Role).Value;
            //ViewBag.Akses = roleId;
            ViewBag.Export = db.CheckPermission("HML MCDFA Perorangan Export", DataEntities.PermissionMessageType.NoMessage);

            db.InsertAuditTrail("HMLMCDFAPerorangan_Akses_Page", "Akses Page Dashboard HML MCDFA Perorangan", pageTitle);
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("HML MCDFA Perorangan Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_HMLMCDFAPerorangan", "Export Data", pageTitle);
        ...
        public IActionResult ExportPDF(IFormFile file)
        {
            try
            {
                ...
                var fileName = "HMLMCDFAPerorangan_" + timeStamp + ".pdf";
        ...
        public object GetGridData(DataSourceLoadOptions loadOptions, string memberTypes, string members, string hml, string mcdfa, string periodeAwal, string periodeAkhir, string jenisDebitur)
        {
            ...
            var query = db.getHmlMcdfa("all", "D01", stringMemberTypes, stringMembers, stringHML, stringMCDFA, stringPeriodeAwal, stringPeriodeAkhir, jenisDebitur).ToList();
        ...
        public object GetChartData(DataSourceLoadOptions loadOptions, string memberTypes, string members, string hml, string mcdfa, string periodeAwal, string periodeAkhir, string jenisDebitur)
        {
            ...
            db.Database.CommandTimeout = 420;
            var query = db.getHmlMcdfa("mcdfa", "D01", stringMemberTypes, stringMembers, stringHML, stringMCDFA, stringPeriodeAwal, stringPeriodeAkhir, jenisDebitur).ToList();
