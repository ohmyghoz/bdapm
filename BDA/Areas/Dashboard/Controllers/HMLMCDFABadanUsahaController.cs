...
{
    [Area("Dashboard")]
    public class HMLMCDFABadanUsahaController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public HMLMCDFABadanUsahaController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index()
        {
            ...
            db.CheckPermission("HML MCDFA Badan Usaha View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
            //var roleId = HttpContext.User.FindFirst(ClaimTypes.Role).Value;
            //ViewBag.Akses = roleId;
            ViewBag.Export = db.CheckPermission("HML MCDFA Badan Usaha Export", DataEntities.PermissionMessageType.NoMessage);

            db.InsertAuditTrail("HMLMCDFABadanUsaha_Akses_Page", "Akses Page Dashboard HML MCDFA Badan Usaha", pageTitle);
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("HML MCDFA Badan Usaha Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_HMLMCDFABadanUsaha", "Export Data", pageTitle);
        ...
        public IActionResult ExportPDF(IFormFile file)
        {
            try
            {
                ...
                var fileName = "HMLMCDFABadanUsaha_" + timeStamp + ".pdf";
        ...
        public object GetGridData(DataSourceLoadOptions loadOptions, string memberTypes, string members, string hml, string mcdfa, string periodeAwal, string periodeAkhir, string jenisDebitur)
        {
            ...
            var query = db.getHmlMcdfa("all", "D02", stringMemberTypes, stringMembers, stringHML, stringMCDFA, stringPeriodeAwal, stringPeriodeAkhir, jenisDebitur).ToList();

            return DataSourceLoader.Load(query, loadOptions);
        }

        public object GetChartData(DataSourceLoadOptions loadOptions, string memberTypes, string members, string hml, string mcdfa, string periodeAwal, string periodeAkhir, string jenisDebitur)
        {
            var login = HttpContext.User.FindFirst(ClaimTypes.Name).Value;
            ...
            db.Database.CommandTimeout = 420;
            var query = db.getHmlMcdfa("mcdfa", "D02", stringMemberTypes, stringMembers, stringHML, stringMCDFA, stringPeriodeAwal, stringPeriodeAkhir, jenisDebitur).ToList();
