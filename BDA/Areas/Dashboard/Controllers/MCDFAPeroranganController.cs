...
{
    [Area("Dashboard")]
    public class MCDFAPeroranganController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public MCDFAPeroranganController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index()
        {
            ...
            db.CheckPermission("MCDFA Perorangan View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
            //var roleId = HttpContext.User.FindFirst(ClaimTypes.Role).Value;
            //ViewBag.Akses = roleId;
            ViewBag.Export = db.CheckPermission("MCDFA Perorangan Export", DataEntities.PermissionMessageType.NoMessage);

            db.InsertAuditTrail("MCDFAPerorangan_Akses_Page", "Akses Page Dashboard MCDFA Perorangan", pageTitle);
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("MCDFA Perorangan Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_MCDFAPerorangan", "Export Data", pageTitle);
        ...
        public IActionResult ExportPDF(IFormFile file)
        {
            try
            {
                ...
                var fileName = "MCDFAPerorangan_" + timeStamp + ".pdf";
