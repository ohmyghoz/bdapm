...
{
    [Area("Dashboard")]
    public class KantorCabangController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public KantorCabangController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index(string lvl, string from, string until, string type, string member, string datamart, string ideb)
        {
            ...
            db.CheckPermission("Kantor Cabang View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);

            db.InsertAuditTrail("KantorCabang_Akses_Page", "Akses Page Dashboard Kantor Cabang", pageTitle);

            //var roleId = HttpContext.User.FindFirst(ClaimTypes.Role).Value;
            //ViewBag.Akses = roleId;
            ViewBag.Export = db.CheckPermission("Kantor Cabang Export", DataEntities.PermissionMessageType.NoMessage);
        ...
        [HttpPost]
        public IActionResult Detail(string memberType, string member, string kantorCabang)
        {
            ...
            //var roleId = HttpContext.User.FindFirst(ClaimTypes.Role).Value;
            //ViewBag.Akses = roleId;
            return Json(new { result = "Redirect", url = Url.Action("Detail", "KantorCabang") });
        }
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("Kantor Cabang Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_KantorCabang", "Export Data", pageTitle);
        ...
        public IActionResult LogExportDetail()
        {
            try
            {
                ...
                db.CheckPermission("Kantor Cabang Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportDetail_KantorCabang", "Export Data", pageTitle);
                return Json(new { result = "Success" });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult LogExportInquiry()
        {
            try
            {
                ...
                db.CheckPermission("Kantor Cabang Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportInquiry_KantorCabang", "Export Data", pageTitle);
                return Json(new { result = "Success" });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult Detail()
        {
            ...
            var listGlo = (from q in db.Glossary
                           where q.stsrc == "A" && q.GloPIdeb == pideb && q.GloTipe == "KC" 
                           select q).ToList();
            if (listGlo.Count() != 0)
            {
        ...
        public IActionResult ExportPDF(IFormFile file)
        {
            try
            {
                ...
                var fileName = "KantorCabang_" + timeStamp + ".pdf";
        ...
        #region "GetGridData"
        public object GetGridData(DataSourceLoadOptions loadOptions, string levelAlert, string memberTypes, string members, string periodeAwal, string periodeAkhir, string dataMart, string permintaanIDeb, string offices)
        {
            TempData.Clear();
            ...
            var query = db.getGroupedAlertSummary("KC", null, stringLevelAlert, stringMemberTypes, stringMembers, stringOffices, stringPeriodeAwal, stringPeriodeAkhir, dataMart, permintaanIDeb, null, null, null, null).ToList();
        ...
        public object GetChartData(DataSourceLoadOptions loadOptions)
        {
            ...
            db.Database.CommandTimeout = 420;
            var query = db.getAlertSummary("KC", null, stringLevelAlert, memberType, member, kantorCabang, stringPeriodeAwal, stringPeriodeAkhir, dataMart, permintaanIDeb, null, null, null, null).ToList();
        ...
        public object GetGridRecordData(DataSourceLoadOptions loadOptions, long id)
        {
            try
            {
                ...
                try
                {
                    ...
                    return DataSourceLoader.Load(query, loadOptions);
                    //return DataSourceLoader.Load(JsonConvert.SerializeObject(dt), loadOptions);
                    //return JSONresult;
                }
                catch (Exception ex)
                {
