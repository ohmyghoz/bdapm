...
{
    [Area("Dashboard")]
    public class SummaryAktivitasController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public SummaryAktivitasController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index()
        {
            ...
            db.CheckPermission("Summary Aktivitas View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
            //var roleId = HttpContext.User.FindFirst(ClaimTypes.Role).Value;
            //ViewBag.Akses = roleId;
            ViewBag.Export = db.CheckPermission("Summary Aktivitas Export", DataEntities.PermissionMessageType.NoMessage);

            db.InsertAuditTrail("SummaryAktivitas_Akses_Page", "Akses Page Dashboard Summary Aktivitas", pageTitle);
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("Summary Aktivitas Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_SummaryAktivitas", "Export Data", pageTitle);
        ...
        #region "GetGridData"
        public object GetGridData(DataSourceLoadOptions loadOptions, string memberTypes, string members, string periodeAwal)
        {
            ...
            var query = db.getLogPerHour(stringMemberTypes, stringMembers, stringPeriodeAwal).ToList();
