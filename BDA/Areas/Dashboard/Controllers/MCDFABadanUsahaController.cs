...
{
    [Area("Dashboard")]
    public class MCDFABadanUsahaController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public MCDFABadanUsahaController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index()
        {
            ...
            db.CheckPermission("MCDFA Badan Usaha View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
            //var roleId = HttpContext.User.FindFirst(ClaimTypes.Role).Value;
            //ViewBag.Akses = roleId;
            ViewBag.Export = db.CheckPermission("MCDFA Badan Usaha Export", DataEntities.PermissionMessageType.NoMessage);

            db.InsertAuditTrail("MCDFABadanUsaha_Akses_Page", "Akses Page Dashboard MCDFA Badan Usaha", pageTitle);
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("MCDFA Badan Usaha Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_MCDFABadanUsaha", "Export Data", pageTitle);
        ...
        public IActionResult ExportPDF(IFormFile file)
        {
            try
            {
                ...
                var fileName = "MCDFABadanUsaha_" + timeStamp + ".pdf";
