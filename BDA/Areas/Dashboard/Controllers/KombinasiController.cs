...
{
    [Area("Dashboard")]
    public class KombinasiController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public KombinasiController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index()
        {
            ...
            db.CheckPermission("Kombinasi View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
            //var roleId = HttpContext.User.FindFirst(ClaimTypes.Role).Value;
            //ViewBag.Akses = roleId;
            ViewBag.Export = db.CheckPermission("Kombinasi Export", DataEntities.PermissionMessageType.NoMessage);

            db.InsertAuditTrail("Kombinasi_Akses_Page", "Akses Page Dashboard Kombinasi", pageTitle);
        ...
        [HttpPost]
        public IActionResult Detail(string memberType, string member, string kodeAlert, string kantorCabang, string userId, string frekuensi, string waktu)
        {
            ...
            if (!string.IsNullOrWhiteSpace(kantorCabang))
            {
                TempData["DetailKantorCabang"] = kantorCabang;
            }

            if (!string.IsNullOrWhiteSpace(userId))
            {
                TempData["DetailUserId"] = userId;
            }
            ...
            {
                TempData["DetailWaktu"] = waktu;
            }
            //var roleId = HttpContext.User.FindFirst(ClaimTypes.Role).Value;
            //ViewBag.Akses = roleId;

            return Json(new { result = "Redirect", url = Url.Action("Detail", "Kombinasi") });
        }
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("Kombinasi Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_Kombinasi", "Export Data", pageTitle);
        ...
        public IActionResult LogExportDetail()
        {
            try
            {
                ...
                db.CheckPermission("Kombinasi Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportDetail_Kombinasi", "Export Data", pageTitle);
                return Json(new { result = "Success" });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult LogExportInquiry()
        {
            try
            {
                ...
                db.CheckPermission("Kombinasi Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportInquiry_Kombinasi", "Export Data", pageTitle);
                return Json(new { result = "Success" });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult ExportPDF(IFormFile file)
        {
            try
            {
                ...
                var fileName = "Kombinasi_" + timeStamp + ".pdf";
        ...
        public IActionResult Record(long? id)
        {
            ...
            if (obj.DIMENSI3 != null)
            {
                q = q + "AND ied.user_id='" + obj.DIMENSI3 + "' ";
            }
        ...
        #region "GetGridData"
        public object GetGridData(DataSourceLoadOptions loadOptions, string levelAlert, string jenisAlert, string memberTypes, string members, string periodeAwal, string periodeAkhir, string dataMart, string permintaanIDeb/*, string offices, string frekuensi, string waktu*/)
        {
            TempData.Clear();
            ...
            //string[] Offices = JsonConvert.DeserializeObject<string[]>(offices);
            ...
            //    stringOffices = string.Join(", ", Offices);
            //}
            ...
            //if (waktu != null)
            //{
            //    TempData["Waktu"] = waktu;
            //}
            db.Database.CommandTimeout = 420;
            //var query = db.getGroupedAlertSummary("KOM", stringJenisAlert, stringLevelAlert, stringMemberTypes, stringMembers, stringPeriodeAwal, stringPeriodeAkhir, dataMart, permintaanIDeb, stringOffices, null, frekuensi, waktu).ToList();
            var query = db.getGroupedAlertSummary("KOM", stringJenisAlert, stringLevelAlert, stringMemberTypes, stringMembers, null, stringPeriodeAwal, stringPeriodeAkhir, dataMart, permintaanIDeb, null, null, null, null).ToList();
        ...
        public object GetChartData(DataSourceLoadOptions loadOptions)
        {
            ...
            //if (TempData.Peek("DetailKantorCabang") != null)
            //{
            //    kantorCabang = TempData.Peek("DetailKantorCabang").ToString();
            //}

            //if (TempData.Peek("DetailUserId") != null)
            //{
            //    userId = TempData.Peek("DetailUserId").ToString();
            //}
            ...
            //if (TempData.Peek("DetailWaktu") != null)
            //{
            //    waktu = TempData.Peek("DetailWaktu").ToString();
            //}
            ...
            db.Database.CommandTimeout = 420;
            //var query = db.getAlertSummary("KOM", kodeAlert, stringLevelAlert, memberType, memberType + " - " + member, stringPeriodeAwal, stringPeriodeAkhir, dataMart, permintaanIDeb, kantorCabang, userId, frekuensi, waktu).ToList();
            var query = db.getAlertSummary("KOM", kodeAlert, stringLevelAlert, memberType, member, null, stringPeriodeAwal, stringPeriodeAkhir, dataMart, permintaanIDeb, null, null, null, null).ToList();
        ...
        public object GetGridRecordData(DataSourceLoadOptions loadOptions, long id)
        {
            try
            {
                ...
                if (as1.DIMENSI3 != null) {
                    q = q + "AND ied.user_id='" + as1.DIMENSI3 + "' ";
                }
        ...
        #region "RefGetter"
        public IActionResult GetKOMAlertTypes(DataSourceLoadOptions loadOptions)
        {
            var query = db.Alert_Master.Where(x => x.KODE_ALERT.StartsWith("KOM")).Select(x => new { x.KODE_ALERT, x.NAMA_ALERT }).ToList();
        ...
        public IActionResult GetMemberOffices(DataSourceLoadOptions loadOptions, string memberTypes, string members, bool reset)
        {
            if (reset)
            {
                loadOptions.Skip = 0;
            ...
            var query = db.master_office_ljk.Where(x => x.status_aktif == "Y" && x.status_delete == "T" && MemberTypes.Contains(x.kode_jenis_ljk) && Members.Contains(x.kode_ljk)).Select(x => new { x.kode_kantor_cabang, Display = x.kode_kantor_cabang + " - " + x.kantor_cabang });
            return Content(JsonConvert.SerializeObject(DataSourceLoader.Load(query, loadOptions)), "application/json");
        }
