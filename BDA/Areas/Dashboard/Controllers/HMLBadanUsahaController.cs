...
{
    [Area("Dashboard")]
    public class HMLBadanUsahaController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public HMLBadanUsahaController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index()
        {
            ...
            db.CheckPermission("HML Badan Usaha View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
            //var roleId = HttpContext.User.FindFirst(ClaimTypes.Role).Value;
            //ViewBag.Akses = roleId;
            ViewBag.Export = db.CheckPermission("HML Badan Usaha Export", DataEntities.PermissionMessageType.NoMessage);

            db.InsertAuditTrail("HMLBadanUsaha_Akses_Page", "Akses Page Dashboard HML Badan Usaha", pageTitle);
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("HML Badan Usaha Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_HMLBadanUsaha", "Export Data", pageTitle);
        ...
        public IActionResult ExportPDF(IFormFile file)
        {
            try
            {
                ...
                var fileName = "HMLBadanUsaha_" + timeStamp + ".pdf";
        ...
        public object GetGridData(DataSourceLoadOptions loadOptions, string memberTypes, string members, string hml, /*string mcdfa,*/ string periodeAwal, string periodeAkhir, string jenisDebitur)
        {
            ...
            var query = db.getHmlMcdfa("hml", "D02", stringMemberTypes, stringMembers, stringHML, stringMCDFA, stringPeriodeAwal, stringPeriodeAkhir, jenisDebitur).ToList();
