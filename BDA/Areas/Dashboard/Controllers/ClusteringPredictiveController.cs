...
{
    [Area("Dashboard")]
    public class ClusteringPredictiveController : Controller
    {
        private DataEntities db;
        private IWebHostEnvironment _env;

        public ClusteringPredictiveController(DataEntities db, IWebHostEnvironment env)
        {
            this.db = db;
            _env = env;
        }
        ...
        public IActionResult Index()
        {
            ...
            db.CheckPermission("Clustering Predictive nilai Agunan View", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
            ViewBag.Export = db.CheckPermission("Clustering Predictive nilai Agunan Export", DataEntities.PermissionMessageType.NoMessage);

            db.InsertAuditTrail("ClusteringPredictive_Akses_Page", "Akses Page Dashboard Clustering Predictive nilai Aguan", pageTitle);
        ...
        public IActionResult LogExportIndex()
        {
            try
            {
                ...
                db.CheckPermission("Clustering Predictive nilai Agunan Export", DataEntities.PermissionMessageType.ThrowInvalidOperationException);
                db.InsertAuditTrail("ExportIndex_ClusteringPredictive", "Export Data", pageTitle);
        ...
        public IActionResult ExportPDF(IFormFile file)
        {
            try
            {
                ...
                var fileName = "ClusteringPredictive_" + timeStamp + ".pdf";
        ...
        #region "GetGridData"
        public object GetGridData(DataSourceLoadOptions loadOptions, string memberTypes, string members, string periodeAwal, string periodeAkhir, string jenisAgunan, string cluster)
        {
            ...
            string[] JA = JsonConvert.DeserializeObject<string[]>(jenisAgunan);
            string[] Cluster = JsonConvert.DeserializeObject<string[]>(cluster);
            ...
            if (JA.Length > 0)
            {
                stringJA = string.Join(", ", JA);
            }

            if (Cluster.Length > 0)
            {
                stringCluster = string.Join(", ", Cluster);
            }

            db.Database.CommandTimeout = 420;
            var query = db.getDataCluster(stringPeriodeAwal,stringPeriodeAkhir,stringMemberTypes,stringMembers,stringJA,stringCluster, "T").ToList();

            return DataSourceLoader.Load(query, loadOptions);
        }

        [HttpGet]
        public object GetPieChartData(DataSourceLoadOptions loadOptions, string memberTypes, string members, DateTime? periodeAwal, DateTime? periodeAkhir, string jenisAgunan, string cluster)
        {
            string stringPeriodeAwal = null;
            string stringPeriodeAkhir = null;
            ...
            db.Database.CommandTimeout = 420;
            var query = db.getDataCluster(stringPeriodeAwal, stringPeriodeAkhir, memberTypes, stringMembers, jenisAgunan, cluster, "Y").OrderByDescending(x => x.nilai_agunan).Take(10).ToList();

            return DataSourceLoader.Load(query, loadOptions);
        }

        [HttpGet]
        public object GetBarData(DataSourceLoadOptions loadOptions, string memberTypes, string members, DateTime? periodeAwal, DateTime? periodeAkhir, string jenisAgunan, string cluster)
        {
            string stringPeriodeAwal = null;
            string stringPeriodeAkhir = null;
        ...
        public IActionResult GetAgunan(DataSourceLoadOptions loadOptions)
        {
